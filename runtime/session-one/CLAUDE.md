...
4. **After the session ends**, run the post-session feedback protocol (below) and capture reactions in `../../session-records/YYYY-MM-DD-firstname-lastname/feedback.md`. Then run the post-session profile capture step (below) to write the user's multi-page profile.

---

## Hard constraints — what you MUST NOT do

- **Do not reference any files outside this directory.** No external context. No prior conversations. You know only what's in this `session-one/` directory plus what the user says in this session.
...
At the end of the session, write the completed artifact to `../../session-records/YYYY-MM-DD-firstname-lastname/transcript.md` under a `## Final Artifact` heading. Include the full session transcript (their verbatim answers by screen) above the artifact for reference. Then generate the shareable HTML artifact at `../../session-records/YYYY-MM-DD-firstname-lastname/artifact.html` using `artifacts/_template.html` + `artifact-cleanup-guide.md`.
...
- Your generated content (vectors, surprise, Your Name, gaps) with date/timestamp
- Final artifact at the end (under `## Final Artifact` heading)

### `../../session-records/YYYY-MM-DD-firstname-lastname/feedback.md`
- Post-session Q&A verbatim
- Any unsolicited observations they made

### `../../session-records/YYYY-MM-DD-firstname-lastname/artifact.html`
- Shareable HTML artifact generated from `artifacts/_template.html` + `artifact-cleanup-guide.md`
...
## Post-session profile capture

After Session 1 ends and the artifact has been delivered, perform the following capture step. **This is runner-side automation — no user-facing screen.** The user has already received their artifact; this is bookkeeping infrastructure that preserves session data so it's available for future sessions.
