...
- **"Archetype"** — never appears in user-facing copy.
...
(Note: "Profile" / "profile page" IS an OK user-facing word — used in the system architecture and comfortable to readers.)
...
Example: if a user said "Half present" stuck with them most → that becomes Your Name on the lead.
...
**Closer** — the Version-1 + Session 2 promise, kept honest, plus an explicit feedback invitation:
...
Two lines, not one. The first line is the Version-1 + next-session promise. The second line explicitly invites feedback — this matters because users (especially non-technical users) don't assume they can critique something that looks polished. The invitation has to be stated, not implied.

Do not promise Session 2 if there is no intent to deliver it. Do not soften the "Version 1" framing — users should know this is iterative.

---

## Checklist (run before sending)

- [ ] No "artifact" / "archetype" / "vector" / "signature" / "leak" / phase-labels in user-facing copy. ("profile" / "profile page" IS an OK user-facing word.)
...
- [ ] Surprise is the final landed version only — concrete user-examples + plain contrast (not abstract-mechanism)
- [ ] Gap questions framed as "next time" with explicit permission not to hold them now
- [ ] Opener uses forward-looking framing ("current view that will evolve") — not defensive "keep/change/reject" framing
- [ ] Your Name has "does this sound right?" framing — not a "keep/say no/rename" 3-option choice
- [ ] Closer is the Version-1 + next-time honest promise
